...
func move_enemy():
	...
	if blast_enemies:
	...
	elif in_hitbox and player_attack_global and can_track_player:
		attack_player()
		...
		velocity.y = ENEMY_SPEED*0.1*sin(angle_to_player) * rnd_y
		velocity.x  = ENEMY_SPEED*0.1*sin(angle_to_player + PI/2) * rnd_x
#	elif can_attack and can_track_player:
#		velocity = dir * ENEMY_SPEED*1.5
#		if player.has_double_jumped:
#			velocity.y = velocity.y/3
#	elif player_attack_global and can_track_player:
#		velocity = dir * ENEMY_SPEED*1.5
#		if player.has_double_jumped:
#			velocity.y = velocity.y/3
#	elif chase_player and can_track_player:
#		velocity = dir * ENEMY_SPEED
	else:
		var diff = new_position
		var angle_to_ball = diff.angle()
		velocity.y = ENEMY_SPEED*sin(angle_to_ball)
		velocity.x = ENEMY_SPEED*sin(angle_to_ball + PI/2)
...
func make_path():
	if can_attack and can_track_player:
